...
local ChannelBarHooked = false
local HideChannelBar = function(ChannelBar)
	ChannelBar = ChannelBar or ExperienceChatApp:FindFirstChild("channelBar")

	if (ChannelBar and ChannelBar.Name == "channelBar") then
		pcall(function()
...
local HookChannelBar = function()
	HideChannelBar()

	if (ChannelBarHooked) then
		return
	end

	ChannelBarHooked = true
	Connect(ExperienceChatApp.ChildAdded, function(Object)
		if (Object.Name == "channelBar") then
			Defer(HideChannelBar, Object)
		end
	end)
end
...
if (Configuration.OldPlayerList) then
	...
	local AddPlayer = function(Player)
		...
		local UpdateStats = function()
			...
			if (TeamFrame and TeamFrame.UpdateTeamStats)
